...
// AddProduct.js - UPDATED WITH ENHANCED SUCCESS HANDLING AND AUTO-REFRESH
import React, { useState, useEffect } from "react";
import axios from "axios";
import MDEditor from '@uiw/react-md-editor';
import "./AddProduct.css";
...
export default function AddProduct() {
    ...
    // ✅ NEW: Success state with enhanced UI
    const [showSuccess, setShowSuccess] = useState(false);
    ...
    // ✅ ENHANCED: Submit function with proper form reset and auto-refresh option
    const submit = async e => {
        e.preventDefault();
        if (!validateForm()) return;
        ...
        // Append all form fields including arrays as JSON strings to avoid parse errors
        Object.entries(form).forEach(([key, val]) => {
            if (Array.isArray(val)) {
                // Send arrays as JSON strings to avoid parse errors
        ...
        try {
            ...
            if (res.data.success) {
                ...
                setAddedProductId(res.data.product?.id);

                // Show success message
                setSuccessMessage(`Product "${form.name}" has been added successfully!`);
                setShowSuccess(true);

                // Reset the entire form
                resetForm();

                // Auto-refresh the page after 5 seconds (optional)
    ...
    // ✅ NEW: Function to add another product (reset form without page refresh)
    const addAnotherProduct = () => {
        if (autoRefreshTimer) {
            clearTimeout(autoRefreshTimer);
    ...
    // ✅ NEW: Function to cancel auto-refresh and stay on page
    const stayOnPage = () => {
        if (autoRefreshTimer) {
            clearTimeout(autoRefreshTimer);
            setAutoRefreshTimer(null);
    ...
    // ✅ NEW: Function to view the added product
    const viewProduct = () => {
        if (addedProductId) {
            window.location.href = `/products/${addedProductId}`;
        } else {
    ...
    return (
        <div className="premium-container">
            {/* ✅ ENHANCED: Success Message with Auto-refresh Countdown */}
            {showSuccess && (
                <div className="success-overlay">
                    <div className="success-message">
                        <div className="success-icon">
                        ...
                        <h3 className="success-title">Product Added Successfully!</h3>
                        <p className="success-subtitle">{successMessage}</p>
                        <div className="success-actions">
                            <button
                                className="success-btn primary"
                                onClick={addAnotherProduct}
                        ...
                        {autoRefreshTimer && (
                            <div className="auto-refresh-info">
                                <p>Page will auto-refresh in 5 seconds to show updated product list...</p>
                                <button
                                    className="cancel-refresh-btn"
                                    onClick={stayOnPage}
                                >
                                    Cancel Auto-refresh
                                </button>
            ...
            <div className="premium-card">
                ...
                <form className="premium-form" onSubmit={submit}>
                    ...
                    {/* Product Images Section - Multiple Image Upload */}
                    <div className="form-section">
                        <h3 className="section-title">Product Images</h3>
                        <div className="form-group">
                            <label htmlFor="images" className="form-label">
