...
// // auth.js - Updated Google Auth Route
...
// auth.js - Complete Updated with 2FA Login Support and UUID
// FIXED: Google auth route now properly checks 2FA status
...
// ============================================
// FIXED GOOGLE AUTH ROUTE WITH PROPER 2FA HANDLING
// ============================================
...
router.post('/auth/google', async (req, res) => {
    ...
    try {
        ...
        if (existing.length === 0) {
            ...
            // For new user, 2FA is not enabled by default
            const authToken = generateJWT({ userId: user.id, email: user.email });
